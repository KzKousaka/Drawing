#version 400

//
//  main.frag
//  Drawing
//
//  Created by Kazuma Kousaka on 2018/03/20.
//
in  vec4    color;
in  vec2    uv;
out vec4    oColor;
float pi = 3.1415;

vec3 hsv2rgb(vec3 c)
{
    vec4 K = vec4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
    vec3 p = abs(fract(c.xxx + K.xyz) * 6.0 - K.www);
    return c.z * mix(K.xxx, clamp(p - K.xxx, 0.0, 1.0), c.y);
}


vec3 screen(vec3 c1, vec3 c2) {
    return vec3(1)-(vec3(1)-c1) * (vec3(1)-c2);
}

float sinWave(float r, float r2, float r3){
    float y = (sin(r)*0.01+sin(r2)*0.3+sin(r3)*0.05+1)*0.5;
    return 1-abs(y - uv.y);
}

float calcAngle(vec2 center, vec2 pos){
    vec2 s = center-pos;
    float len = distance(center, pos);
    float result = asin(s.y/len);
    if(s.x < 0){
        result = pi*2-result;
    }
    return result;
}

vec3 diamond(vec2 p){
    float powNum = 50;
    vec3 color = vec3(0);
    color = screen(color, vec3(pow(1-abs(0.5-p.x - p.y), powNum)));
    color = screen(color, vec3(pow(1-abs(p.x - p.y+0.5), powNum)));
    color = screen(color, vec3(pow(1-abs(1-p.x - p.y+0.5), powNum)));
    color = screen(color, vec3(pow(1-abs(p.x - p.y-0.5), powNum)));
    return color;
}

void main( void ) {
    
    vec3    color = vec3(0);
    vec2    p = uv;
    vec2    center = vec2(0.5,0.5);
    float   len = distance(center, p) * 0.5;
    float   angle = calcAngle(center, p);
    
    for( int i = 0 ; i < 20 ; i++) {
        angle += pi * 0.04;
        len *= 1.4;
        p = vec2(sin(angle), cos(angle)) * len;
        
        float c = clamp(diamond(p).r, 0, 1);
        color = screen(color, hsv2rgb(vec3(i/10.0, 1, c)) * 0.5);
    }
    
    oColor = vec4(color.rgb, 1);
}
